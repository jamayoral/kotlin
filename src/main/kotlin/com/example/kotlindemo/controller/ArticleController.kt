...
class ArticleController(private val articleRepository: ArticleRepository) {
    ...
    @GetMapping("/articles-web")
    fun getAllArticlesWeb(): Flux<Article> = articleRepository.findAll().toFlux()
    ...
    @GetMapping("/articles-web/{id}")
    fun getArticleByIdWeb(@PathVariable(value = "id") articleId: Long): Mono<ResponseEntity<Article>> {
        return Mono.just(articleRepository.findById(articleId).map { article ->
            ResponseEntity.ok(article)
        }.orElse(ResponseEntity.notFound().build()))
    }
    ...
    @PutMapping("/articles-web/{id}")
    fun updateArticleByIdWeb(@PathVariable(value = "id") articleId: Long,
                             @Valid @RequestBody newArticle: Article): Mono<ResponseEntity<Article>> {

        return Mono.just(articleRepository.findById(articleId).map { existingArticle ->
            val updatedArticle: Article = existingArticle
                    .copy(title = newArticle.title, content = newArticle.content)
            ResponseEntity.ok().body(articleRepository.save(updatedArticle))
        }.orElse(ResponseEntity.notFound().build()))
